...
function show(io::IO, x::fq_nmod_poly)
   if length(x) == 0
      ...
      cstr = ccall((:fq_nmod_poly_get_str_pretty, :libflint), Ref{UInt8}, 
                  (Ref{fq_nmod_poly}, Ref{UInt8}, Ref{FqNmodFiniteField}),
...
function factor_distinct_deg(x::fq_nmod_poly)
   ...
   ccall((:fq_nmod_poly_factor_fit_length, :libflint), Void,
         (Ref{fq_nmod_poly_factor}, Int, Ref{FqNmodFiniteField}),
         tmp, degree(x), F)
   ccall((:fq_nmod_poly_factor_distinct_deg, :libflint), Void,
         (Ref{fq_nmod_poly_factor}, Ref{fq_nmod_poly}, Ref{Int},
         Ref{FqNmodFiniteField}), fac, x, tmp.exp, F)
