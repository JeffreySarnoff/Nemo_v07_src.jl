...
function acb_vec(n::Int)
   return ccall((:_acb_vec_init, :libarb), Ref{acb_struct}, (Int,), n)
end

function acb_vec(b::Array{acb, 1})
   v = ccall((:_acb_vec_init, :libarb), Ref{acb_struct}, (Int,), length(b))
   for i=1:length(b)
       ccall((:acb_set, :libarb), Void, (Ref{acb_struct}, Ref{acb}),
           v + (i-1)*sizeof(acb_struct), b[i])
   end
   return v
end

function array(R::AcbField, v::Ref{acb_struct}, n::Int)
   r = Array{acb}(n)
   for i=1:n
       r[i] = R()
       ccall((:acb_set, :libarb), Void, (Ref{acb}, Ref{acb_struct}),
           r[i], v + (i-1)*sizeof(acb_struct))
   end
   return r
end

function acb_vec_clear(v::Ref{acb_struct}, n::Int)
   ccall((:_acb_vec_clear, :libarb), Void, (Ref{acb_struct}, Int), v, n)
end
...
function from_roots(R::AcbPolyRing, b::Array{acb, 1})
   ...
   ccall((:acb_poly_product_roots, :libarb), Void,
                (Ref{acb_poly}, Ref{acb_struct}, Int, Int), z, tmp, length(b), prec(R))
...
function evaluate_fast(x::acb_poly, b::Array{acb, 1})
   ...
   ccall((:acb_poly_evaluate_vec_fast, :libarb), Void,
                (Ref{acb_struct}, Ref{acb_poly}, Ref{acb_struct}, Int, Int),
...
function interpolate_newton(R::AcbPolyRing, xs::Array{acb, 1}, ys::Array{acb, 1})
   ...
   ccall((:acb_poly_interpolate_newton, :libarb), Void,
                (Ref{acb_poly}, Ref{acb_struct}, Ref{acb_struct}, Int, Int),
...
function roots(x::acb_poly; target=0, isolate_real=false, initial_prec=0, max_prec=0, max_iter=0)
    ...
    while true
        ...
        if isolated == deg
            ...
            if target > 0
                for i = 0 : deg-1
                    re = ccall((:acb_real_ptr, :libarb), Ref{arb_struct},
                        (Ref{acb}, ), roots + i * sizeof(acb_struct))
                    im = ccall((:acb_imag_ptr, :libarb), Ref{arb_struct},
                        (Ref{acb}, ), roots + i * sizeof(acb_struct))
                    t = ccall((:arb_rad_ptr, :libarb), Ref{mag_struct}, (Ref{arb}, ), re)
                    u = ccall((:arb_rad_ptr, :libarb), Ref{mag_struct}, (Ref{arb}, ), im)
                    ok = ok && (ccall((:mag_cmp_2exp_si, :libarb), Cint,
                        (Ref{mag_struct}, Int), t, -target) <= 0)
                    ok = ok && (ccall((:mag_cmp_2exp_si, :libarb), Cint,
                        (Ref{mag_struct}, Int), u, -target) <= 0)
                end
            end

            if isreal(x)
                real_ok = ccall((:acb_poly_validate_real_roots, :libarb),
                    Bool, (Ref{acb_struct}, Ref{acb_poly}, Int), roots, x, wp)
                ...
                if real_ok
                    for i = 0 : deg - 1
                        im = ccall((:acb_imag_ptr, :libarb), Ref{arb_struct},
                            (Ref{acb}, ), roots + i * sizeof(acb_struct))
                        if ccall((:arb_contains_zero, :libarb), Bool, (Ref{arb_struct}, ), im)
                            ccall((:arb_zero, :libarb), Void, (Ref{arb_struct}, ), im)
                        end
                    end
                end
            end
...
function roots_upper_bound(x::acb_poly)
   ...
   p = prec(base_ring(x))
   t = ccall((:arb_rad_ptr, :libarb), Ref{mag_struct}, (Ref{arb}, ), z)
   ccall((:acb_poly_root_bound_fujiwara, :libarb), Void,
         (Ref{mag_struct}, Ref{acb_poly}), t, x)
   s = ccall((:arb_mid_ptr, :libarb), Ref{arf_struct}, (Ref{arb}, ), z)
   ccall((:arf_set_mag, :libarb), Void, (Ref{arf_struct}, Ref{mag_struct}), s, t)
   ccall((:arf_set_round, :libarb), Void,
         (Ref{arf_struct}, Ref{arf_struct}, Int, Cint), s, s, p, ARB_RND_CEIL)
   ccall((:mag_zero, :libarb), Void, (Ref{mag_struct},), t)
   return z
end
